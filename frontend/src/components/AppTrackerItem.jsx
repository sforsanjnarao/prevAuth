...
import { deleteAppEntry } from "../api/appTrackerApi"; // API function
...
const DataPill = ({ category }) => {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium border ${colorClasses}`}
    >
      {category}
    </span>
  );
...
const RiskSlider = ({ score }) => {
  ...
  return (
    <div
      ...
      title={`Risk Level: ${level} (${score}/${MAX_RISK_SCORE})`}
    >
...
function AppTrackerItem({ entry, onDelete }) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error.message || `Failed to delete entry "${entry.appName}".`
      );
      ...
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200 flex flex-col justify-between hover:shadow-md transition-shadow duration-200">
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {entry.dataShared?.map((category) => (
              <DataPill key={category} category={category} />
            )) ?? (
        ...
        {isExpanded && (
          <div className="border-t border-gray-200 mt-3 pt-3 space-y-2 text-sm text-gray-700 animate-fade-in">
            {" "}
            {/* Simple fade-in */}
            {entry.appUrl && (
            ...
            {entry.appCategory && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  Category:{" "}
                  <span className="font-medium">{entry.appCategory}</span>
                </span>
              </div>
            )}
            {entry.notes && (
